...
class MIMICDataPreparator:
    ...
    def load_core_tables(self):
        ...
        self.patients = pd.read_csv(f"{self.mimic_path}/PATIENTS.gz.csv").rename(columns=str.upper)
        self.admissions = pd.read_csv(f"{self.mimic_path}/ADMISSIONS.gz.csv").rename(columns=str.upper)
        
        # Medication data
        self.prescriptions = pd.read_csv(f"{self.mimic_path}/PRESCRIPTIONS.gz.csv").rename(columns=str.upper)
        
        # Clinical notes (for comorbidity detection)
        self.noteevents = pd.read_csv(f"{self.mimic_path}/NOTEEVENTS.gz.csv").rename(columns=str.upper)
        
        # Lab events (for clinical indicators)
        self.labevents = pd.read_csv(f"{self.mimic_path}/LABEVENTS.gz.csv").rename(columns=str.upper)
        
        # Lab items dictionary for lab names
        try:
            self.d_labitems = pd.read_csv(f"{self.mimic_path}/D_LABITEMS.gz.csv").rename(columns=str.upper)
        ...
        self.diagnoses = pd.read_csv(f"{self.mimic_path}/DIAGNOSES_ICD.gz.csv")
